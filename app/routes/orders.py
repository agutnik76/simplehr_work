...
def create_order():
    ...
    if form.validate_on_submit():
            # Create new order
        order = Order()
        order.employee_id=2 #form.employee_id.data,
        order.type='hire' #form.type.data,
        order.date_issued=form.date_issued.data  #.strftime('%Y-%m-%d')
        ...
        try:
            ...
            db.session.rollback()
            print(f"Ошибка при создании приказа {e}")
...
def edit_order(id):
    ...
    form = OrderForm(order_id=id, obj=order)
