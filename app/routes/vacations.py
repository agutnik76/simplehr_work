...
from app.forms import VacationForm, Counter
    ...
    try:
        ...
        for vacation in vacations:
            ...
            while current_date <= end_range:
                vacationTemp = vacation_calendar.get(current_date)
                if not vacationTemp:
        ...
        return render_template('vacations/calendar.html',
                               ...
                               end_date=end_date,
                               date=date, logger=log_to_logger)  # Pass the date function to the template
    except Exception as e:
        ...
        if form.validate_on_submit():
            ...
            try:
                ...
                if employee:
                    ...
                    notify_vacation_created(current_user.id, employee.full_name, vacation_type_name,
                                            form.start_date.data)
                    # Логируем действие
                    log_vacation_create(vacation.id, employee.full_name, form.type.data,
                                        form.start_date.data.strftime('%d.%m.%Y') if form.start_date.data else '',
                                        current_user.id)
