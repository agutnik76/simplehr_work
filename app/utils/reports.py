    ...
    try:
        ...
        for vacation in vacations:
            try:
                ...
                print(f"vacation.start_date {str(vacation.start_date)}")
                print(f"vacation.end_date {str(vacation.end_date)}")
                print(f"first {str(first_day)}")
                print(f"last {str(last_day)}")
...
def generate_turnover_report(period_days=365):
    ...
    try:
        ...
        print(f"turnover 1")
        print(f"turnover data {end_date}")
        print(f"turnover data {start_date}")
        # Группируем по месяцам
        monthly_turnover = {}
        for employee in dismissed_employees:
            try:
                print(f"for data {employee}")
                month_key = employee.hire_date.strftime('%Y-%m')
                ...
                if employee.status == 'active':
                    monthly_turnover[month_key]['hired'] += 1
                ...
                print(f"turnover exception {str(e)}")
