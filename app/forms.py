...
from app.models import Employee, Department, Position, User, Vacation, Order
from app import db
...
class OrderForm(FlaskForm):
    employee_id = SelectField('Сотрудник', coerce=safe_int_coerce, validators=[DataRequired()])
    ...
    new_department_id = SelectField('Новое подразделение', coerce=safe_int_coerce, validators=[Optional()])
    # new_position_id = SelectField('Новая должность', coerce=int, validators=[Optional()])
    new_position_id = SelectField('Новая должность', coerce=safe_int_coerce, validators=[Optional()])
    ...
    def validate_date_issued(self, date_issued):
        ...
        try:
            ...
            print(f"model: date exception {str(e)}")
